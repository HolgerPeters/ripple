...
module Ripple
  ...
  CONFIG_FILE = '_ripple.yml'.freeze
  ...
  def self.find_config_file(source_dir)
    ...
    while (fn = File.join(dir, CONFIG_FILE)) && !File.exist?(fn)
  ...
  def self.process(opts = {})
    works(configuration(opts)).each(&:process)
  end
  ...
  def self.format_movement_title(mvt)
    if mvt =~ /^(\d+)\-(.+)$/
      ...
      name = Regexp.last_match(2).tr('-', ' ').gsub(/\b('?[a-z])/) { Regexp.last_match(1).capitalize }
      "#{num}. #{name}"
    else
      mvt
    end
  ...
  def self.version
    yml = load_yaml(File.join(File.dirname(__FILE__), '..', 'VERSION.yml'))
