module Ripple
  class Compilation
    ...
    def qualify_movement(work, mvt)
      found = Dir["#{work}/*"].select do |entry|
        if File.directory?(entry)
          ...
          (n == mvt) || ((n =~ /^(\d+)/) && (Regexp.last_match(1).to_i(10) == mvt.to_i))
        end
      end.first
    ...
    def qualify_all_movements
      @config['movements'].each do |m|
        mvt = m['movement']
        if mvt =~ /(.+)#(.+)/
          m['work'] = Regexp.last_match(1)
          m['movement'] = qualify_movement(Regexp.last_match(1), Regexp.last_match(2))
          unless m['movement']
    ...
    def compute_movement_titles
      @config['movements'].each_with_index do |m, i|
        ...
        mvt = m['movement'] =~ /^(\d+)\-(.+)/ ?
          "#{Regexp.last_match(1).to_i} - #{Regexp.last_match(2).titlize(true)}" : m['movement']
        m['title'] ||= format('%d. %s/%s', i + 1, w.name, mvt)
      end
    end
    ...
    def calculate_fast_compile(config)
      ...
      mvts = @config['fast-compile'].each_with_object([]) do |specifier, m|
        if specifier =~ /(.+)#(.+)/
          work = Regexp.last_match(1)
          if mvt = qualify_movement(work, Regexp.last_match(2))
            m << {
              'work' => work,
              'movement' => mvt
            }
    ...
    def name
      n = (@path =~ /([^\/]+)$/) && Regexp.last_match(1)
      n = (n =~ /^(.+)\.yml$/) && Regexp.last_match(1)
    end
    ...
    def clean_config
      @config.reject { |k, _v| k == 'movements' || k == 'compile' }
    end

    def relative_path
      root = File.expand_path(config['source'])
      p = (path =~ /^(.+)\.yml$/) && Regexp.last_match(1)
      p =~ /^#{root}\/(.+)$/ ? Regexp.last_match(1) : '.'
    end
    ...
    def process
      parts = if selected_parts = @config['selected_parts']
                selected_parts.split(',')
              else
                all_parts
              end
  ...
  class CompilationScore < Score
    ...
    def midi_filename(mvt)
      mvt = 'score' if mvt.nil? || mvt.empty?
