...
class PDFTK
  class << self
    ...
    def info(fn)
      i = exec("#{fn} dump_data")
      a = nil
      i.lines.each_with_object({}) do |l, m|
        case l
        when /^InfoKey: (.*)/
          a = Regexp.last_match(1)
        when /^InfoValue: (.*)/
          m[a] = Regexp.last_match(1)
        when /^(NumberOfPages): (.*)/
          m[:pages] = Regexp.last_match(2).to_i
          m[Regexp.last_match(1)] = Regexp.last_match(2)
        when /(.+): (.*)/
          m[Regexp.last_match(1)] = Regexp.last_match(2)
        end
      end
    ...
    def booklet_fn(fn)
      if fn =~ /^(.+)\.pdf$/
        "#{Regexp.last_match(1)}-booklet.pdf"
      else
