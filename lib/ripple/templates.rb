  ...
  module Templates
    ...
    def self.staff_groups(parts, config)
      ...
      groups.each do |g|
        ...
        next unless (group_parts.size > 1) && (idx = grouped.array_index(group_parts))
    ...
    def self.should_break(config)
      return config['breaks'] && config['breaks'] > 0 if config['compiled']
    ...
    def self.movement_blank_pages(config)
      return config['breaks'].to_i - 1 if config['compiled']
    ...
    SOLO_TAG = %w[soloText soloIIText soloIIIText soloIVText].freeze
    ...
    def self.render_part(content, config)
      ...
      include += config['part_include'] if config['part_include']
    ...
    def self.render_score(content, config)
      # include files
      include = config['include'] || []
      include += config['score_include'] if config['score_include']
